...
const postJob = async(req, res, next) => {
  ...
  try {
    ...
    if (company.role == "basic") {
      ...
      let jobsPosted = await Job.find({ companyId: company.company_id, advertisedDate: {$gte: start, $lt: end} }).count();

      let jobPostLimit = 10;
      // let subscription = await Subscription.find({ company })
    ...
    let job = await new Job({
      ...
      postedBy: company.role,
      updatedAt: ""
    }).save()
  
    //const msg = formateForMail.formateForMail('postJob', job.jobTitle);
    //nodeMailer.nodeMailer(company.email, 'New Job Posted', msg);
